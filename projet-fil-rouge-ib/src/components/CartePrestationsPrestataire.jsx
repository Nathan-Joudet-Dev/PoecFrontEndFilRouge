...
const CartePrestationsPrestataire = ({ prestation }) => {
  ...
  useEffect(() => {
    ...
    async function fetchPrestataire(id) {
      ...
      const panierTmp = await _service.recupererPanierPrestataire(
        prestataireTmp
      );
      setPanier(panierTmp);
      prestation.validationPrestataire == true
        ? setPrestationTerminee(true)
        : setPrestationTerminee(false);
    }
    fetchPrestataire(+id);
  }, []);
    ...
    <div className="cartePrestationPanier cartePrestataire">
      ...
      <div className="infosPrestation">
        <div className="FirstRow">
          ...
          <span
            className="spanNoteMoyenne"
            hidden={prestation.etat != "Archivée"}
          >
            Note moyenne : {prestation.noteMoyenne}/5{" "}
          </span>
        </div>
        {!prestation.devis.coutTotal && (
        ...
        {!prestationTerminee && (
          <button
            className="terminer"
            onClick={ValiderFinPrestation}
            hidden={prestation.etat != "En cours"}
          >
        ...
        <div className="boutonsAccepterRefuser">
          <button
            className="accepterService"
            onClick={accepterService}
            hidden={prestation.etat != "Demande envoyée"}
          >
            Accepter
          </button>
          <button
            className="refuserService"
            onClick={refuserService}
            hidden={prestation.etat != "Demande envoyée"}
          >
            Refuser
          </button>
          <button
            className="accepterService"
            onClick={creerDevis}
            hidden={prestation.etat != "En attente du devis"}
          >
            Créer le devis
          </button>
          <button
            className="accepterService"
            hidden={prestation.etat != "En attente d'acceptation du devis"}
          >
            Devis envoyé
          </button>
        </div>
